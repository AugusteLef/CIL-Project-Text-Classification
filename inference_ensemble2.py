...
def main(args):
    ...
    list_models = []
    model_state_dict_bart = models.HuggingfaceModel(AutoModelForSequenceClassification.from_pretrained(args.configs[0])).state_dict()
    list_models.append(models.BartModelForEnsemble(model_state_dict_bart))
    model_state_dict_bert = models.HuggingfaceModel(AutoModelForSequenceClassification.from_pretrained(args.configs[1])).state_dict()
    list_models.append(models.BertModelForEnsemble(model_state_dict_bert))
    model_state_dict_bertweet = models.HuggingfaceModel(AutoModelForSequenceClassification.from_pretrained(args.configs[2])).state_dict()
    list_models.append(models.BertweetModelForEnsemble(model_state_dict_bertweet))
    model_state_dict_xlnet = models.HuggingfaceModel(AutoModelForSequenceClassification.from_pretrained(args.configs[3])).state_dict()
