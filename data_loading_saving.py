...
def load_raw_data_trainingset(path: str, label: int) -> DataFrame:
...
def load_raw_data_testset(path: str) -> DataFrame:
...
def concat_DataFrame(df1: DataFrame, df2: DataFrame) -> DataFrame:
...
def write_to_text(path: str, df: DataFrame) -> None:
...
def shuffle_df(df: DataFrame) -> DataFrame:
    """shuffle a DataFrame
